...
def checkBoxInFinish():
    ...
    for fin in finish:
        fin[3] = False  # Каждый третий элемент (наличие ящика в точке сбора) в списке точек наличия устанавливаем ЛОЖЬ
                        # Чтобы в дальнейшем снова проверить каждую точку и общим логическим произведением получить результат
    ...
    while (fin < len(finish) and win): # Для утверждения не победы достаточно чтобы хотя бы одна точка сбора пустовала
        box = 0
        while (box < len(boxes)):
            if (finish[fin][0:2] == boxes[box][0:2]): # Координаты точки сбора и ящика
                finish[fin][3] = True
                box = len(boxes)
            box += 1
